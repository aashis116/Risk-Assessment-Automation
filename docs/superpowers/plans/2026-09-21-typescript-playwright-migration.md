...
test('@live full vendor onboarding through ticket pipeline', async ({ page }) => {
  ...
  await test.step('SRS Scoring', () => {
    if (!onboarding.vendorId) throw new Error('Onboarding did not yield a vendorId; aborting chain.');
    return runSrsScoring(page, { vendorName: vendor_name, vendorId: onboarding.vendorId, provider: 'all' });
  });

  await test.step('FAIR Analysis', () => {
    if (!onboarding.vendorId) throw new Error('Onboarding did not yield a vendorId; aborting chain.');
    return runFairAnalysis(page, { vendorId: onboarding.vendorId });
  });
